...
mod1_int_aug <- augment(mod1_int) %>%
  mutate(.stdresid = resid(mod1_int, 
                           type = "pearson", 
                           scaled = T)) %>%
 # To integrate seeding rate to the data
   left_join(reg_dfw)
...
ggplot(mod1_int_aug, 
  ...
  geom_point(size = 3, 
             alpha = .7) +
  geom_line(data = mod1_int_aug, aes(y = .fixed))
```
...
Bootstrapping is best suited where the original data is 

Bootstrapping is also referred to as **sampling with replacement**. So for ex., if your initial sample has 10 observations, bootstrapping will take a total of 10 samples on each run where:
- takes the first sample (say row 3)
- takes the second sample (say, row 5)
- takes third sample (say, row 7)
- takes fourth sample (say, row 9)
